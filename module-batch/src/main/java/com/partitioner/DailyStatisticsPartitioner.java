...
public class DailyStatisticsPartitioner implements Partitioner {
	...
	public DailyStatisticsPartitioner(DataSource dataSource, int gridSize, LocalDate date) {
	...
	private long getMinVideoId() {
		Long minId = jdbcTemplate.queryForObject(
			"SELECT COALESCE(MIN(video_id), 0) FROM watch_history WHERE created_at = ?",
			Long.class,
			date
		);
	...
	private long getMaxVideoId() {
		Long maxId = jdbcTemplate.queryForObject(
			"SELECT COALESCE(MAX(video_id), 0) FROM watch_history WHERE created_at = ?",
			Long.class,
			date
		);
