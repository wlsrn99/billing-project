...
public class DailyBillingPartitioner implements Partitioner {
	...
	public DailyBillingPartitioner(DataSource dataSource, int gridSize, LocalDate date) {
	...
	private long getMinVideoId() {
		Long minId = jdbcTemplate.queryForObject(
			"SELECT COALESCE(MIN(video_id), 0) FROM video_statistics WHERE date = ?",
			Long.class,
			date
		);
	...
	private long getMaxVideoId() {
		Long maxId = jdbcTemplate.queryForObject(
			"SELECT COALESCE(MAX(video_id), 0) FROM video_statistics WHERE date = ?",
			Long.class,
			date
		);
